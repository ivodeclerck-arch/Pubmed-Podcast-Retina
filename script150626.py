...
# Title patterns marking reply/comment correspondence (case-insensitive).
# "Re:" is handled separately as it needs word-boundary matching.
...
def passes_quality_filter(paper, min_abstract_words=100):
    """Return (ok, reason). Drops reply/comment titles and thin/missing abstracts."""
    title = paper.get("title", "")
    if _REPLY_TITLE_PATTERNS.search(title):
        return False, "reply/comment title"

    abstract = paper.get("abstract", "") or ""
    if abstract.strip() in ("", "(No abstract available.)"):
